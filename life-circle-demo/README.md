...
默认入口已接入后端自适应网格算法，支持选点、获取当前位置、地点搜索、任务进度、取消、质量提示，以及含孔洞和多分量的 BD09LL 几何；真实 Provider 下消费设施列表、分类 Marker 与图例、分类筛选、采样点评估及中心到设施的步行路线（有限检索，见契约第 8 节）。main 新增的 POI data v1 与 OSM 离线路由属后端数据/算法基建，**当前前端不直接调用**（详见契约第 8 节“后端存在、当前前端不直接调用”）。main 已包含国定一社区真实步行链路、稳定性与算法比较记录，见 [后端当前进度](../backend/docs/当前进度与接口报告.md)；本前端自动验收使用合成 Provider 和离线地图 SDK 替身，不替代真实社区精度验证。
