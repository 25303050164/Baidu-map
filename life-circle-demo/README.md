...
默认入口已接入后端自适应网格算法，支持选点、获取当前位置、地点搜索、任务进度、取消、质量提示，以及含孔洞和多分量的 BD09LL 几何。设施统计尚未接入。main 已包含国定一社区真实步行链路、稳定性与算法比较记录，见 [后端当前进度](../backend/docs/当前进度与接口报告.md)；本前端自动验收使用合成 Provider 和离线地图 SDK 替身，不替代真实社区精度验证。
...
已具备真实地图、等时圈任务、部分报告和独立 Demo 流程。正式设施统计、服务盲区、登录、持久化历史、文件导出及综合评分尚未接入。自动验收证明流程与契约，不代表真实社区完整体检或分析准确性。
