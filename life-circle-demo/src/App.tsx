...
export default function App() {
  ...
  useEffect(()=>{const media=window.matchMedia('(max-width: 1050px)');const change=()=>{setCompact(media.matches);if(!media.matches)setMobilePanel(null);};media.addEventListener('change',change);return()=>media.removeEventListener('change',change);},[]);
  const [report,setReport]=useState(false),[mobilePanel,setMobilePanel]=useState<'analysis'|'results'|null>(null),[tab,setTab]=useState<string>('问题区域');
  ...
  const resultPanel=<>
    ...
    <div className={styles.listArea}>
      ...
      </>:<><div className={styles.listCaption}>设施点位 <span>圈外参考单独标识</span></div>{filterFacilities(result,state.filter).map(f=><button key={f.id} className={styles.facilityRow} onClick={()=>{setLayers(l=>({...l,facilities:true}));setFocus({...f,detail:`${categoryMeta[f.category].label} · ${f.inCircle?'15 分钟圈内（演示）':'圈外参考，不计入圈内数量'}`,nonce:Date.now()});setMobilePanel(null);}}><span className={styles.facilityIcon} style={{color:categoryMeta[f.category].color}}>{icons[f.category]}</span><span><strong>{f.name}</strong><small>{categoryMeta[f.category].label} · {f.inCircle?'圈内设施':'圈外参考'}</small></span><EnvironmentOutlined/></button>)}</>}
    </div>
  ...
  return <div className={styles.app}>
    ...
    <div className={styles.mobileToolbar}><Button aria-label="分析设置" icon={<SettingOutlined/>} onClick={()=>setMobilePanel('analysis')}>分析设置</Button><Button aria-label="查看结果" icon={<RadarChartOutlined/>} onClick={()=>setMobilePanel('results')}>查看结果</Button></div>
    <main className={styles.workspace}>
      {!compact&&<aside className={styles.leftPanel}>{analysisPanel}</aside>}
      ...
      {!compact&&<aside className={styles.rightPanel}>{resultPanel}</aside>}
    </main>
    <footer className={styles.footer}><span><InfoCircleOutlined/> 本 Demo 仅展示交互效果，未接入真实地图与算法。</span><span>邻里 · 让社区生活触手可及</span></footer>
    <Drawer title={mobilePanel==='analysis'?'分析设置':'生活圈概览'} open={mobilePanel!==null} placement={mobilePanel==='analysis'?'left':'right'} onClose={()=>setMobilePanel(null)} size={340}>{mobilePanel==='analysis'?analysisPanel:mobilePanel==='results'?resultPanel:null}</Drawer>
    <Drawer title="生活圈体检报告" open={report} onClose={()=>setReport(false)} size={680} extra={<Tag color="gold">演示数据</Tag>}>
      {result&&<article className={styles.report} data-testid="report"><div className={styles.eyebrow}>COMMUNITY CHECKUP / DEMO</div><h1>{result.sample.name}</h1><p className={styles.reportLead}>15 分钟生活圈 · 可视化体检报告</p>{state.dirty&&<Alert type="warning" title="当前设置已变更，本报告仍属于下列原分析条件。" showIcon/>}<dl><div><dt>中心点</dt><dd>{result.sample.center.lng.toFixed(6)}, {result.sample.center.lat.toFixed(6)}</dd></div><div><dt>生成时间</dt><dd>{new Date(result.generatedAt).toLocaleString('zh-CN',{hour12:false})}</dd></div><div><dt>演示场景</dt><dd>{scenarioLabels[result.scenario]}{result.scenario==='failure'?'（重试成功）':''}</dd></div><div><dt>分析口径</dt><dd>15 分钟步行范围与 1 公里盲区分别展示</dd></div><div><dt>数据来源</dt><dd>本地固定模拟数据，无真实更新时间</dd></div><div><dt>评估范围</dt><dd>示意街区预设点位；距离口径尚待正式确认</dd></div></dl>
        <h2>01 / 圈内设施统计</h2><p>全部类别，不受主界面筛选影响。圈外参考不计入。</p><table className={styles.reportTable}><thead><tr><th>设施类别</th><th>圈内数量</th><th>数据状态</th></tr></thead><tbody>{categories.map(c=><tr key={c}><td>{categoryMeta[c].label}</td><td>{summary![c]??'无法确定'}</td><td>{result.quality[c]==='complete'?'演示数据完整':'数据不足'}</td></tr>)}</tbody></table><FacilityChart result={result}/>
