...
const source = (v: unknown) => v === 'synthetic' || v === 'baidu_walking' || v === 'osm_offline' || v === 'hybrid';
const analysisMode = (v: unknown) => v === 'baidu_online' || v === 'osm_offline' || v === 'hybrid';
...
function geometry(v: unknown): boolean {
  if (!object(v) || !['Polygon', 'MultiPolygon'].includes(v.type as string)
    || (v.coordinateSystem !== 'bd09ll' && v.coordinate_system !== 'bd09ll') || !Array.isArray(v.coordinates)) return false;
  const polygons = v.type === 'Polygon' ? [v.coordinates] : v.coordinates;
  return polygons.every(p => Array.isArray(p) && p.length > 0
...
export function validTask(v: unknown): v is TaskStatus {
  return object(v) && typeof v.taskId === 'string' && v.taskId.length > 0
    ...
    && [200, 400, 800].includes(v.budget as number) && finite(v.elapsedSeconds) && v.elapsedSeconds >= 0
     && source(v.dataSource) && (v.analysisMode === undefined || analysisMode(v.analysisMode))
...
export function validResult(v: unknown): v is AnalysisResult {
   if (!object(v) || typeof v.taskId !== 'string' || !v.taskId || !source(v.dataSource)
     || (v.analysisMode !== undefined && !analysisMode(v.analysisMode))
  ...
  if (r.unreachableRegion !== undefined && r.unreachableRegion !== null && !geometry(r.unreachableRegion)) return false;
   if (!validFacilities(v.facilityAnalysis, v.facilitiesStatus, v.data)
     || (v.data.geometry !== null && v.data.geometry !== undefined && !geometry(v.data.geometry))) return false;
  ...
  const s = r.statistics;
   if (!validProvenance(v.provenance, v.dataSource) || !validHybrid(v.hybridResult, v.dataSource)) return false;
...
function validProvenance(value: unknown, dataSource: unknown): boolean {
  if (value === undefined || value === null) return dataSource !== 'osm_offline' && dataSource !== 'hybrid';
  if (!object(value)) return false;
  if (value.osm !== undefined && value.osm !== null) {
    const osm = value.osm;
    if (!object(osm) || typeof osm.participated !== 'boolean'
      || !['ready', 'degraded', 'unavailable'].includes(osm.availability as string)
      || typeof osm.coverageCheckAvailable !== 'boolean' || !Array.isArray(osm.limitations)
      || !osm.limitations.every(item => typeof item === 'string')
      || (osm.coverageBoundary !== null && osm.coverageBoundary !== undefined && !geometry(osm.coverageBoundary))) return false;
  }
  return dataSource !== 'osm_offline' && dataSource !== 'hybrid' || object(value.osm);
}

function validHybrid(value: unknown, dataSource: unknown): boolean {
  if (value === undefined || value === null) return dataSource !== 'hybrid';
  if (dataSource !== 'hybrid' || !object(value) || !object(value.baidu) || !object(value.osm) || !object(value.comparison)) return false;
  return [value.baidu, value.osm].every(branch => typeof branch.participated === 'boolean'
    && source(branch.dataSource) && businessStatus(branch.businessStatus) && object(branch.isochrone))
    && value.comparison.strategy === 'parallel_comparison' && value.comparison.geometryMerged === false;
}
