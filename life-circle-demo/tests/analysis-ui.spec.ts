...
async function setup(page: Page, options: { failOnce?: boolean; unavailable?: boolean; mismatch?: boolean;
  ...
  await page.route('**/*', async route => {
    ...
    return route.fulfill({ status: route.request().method() === 'POST' ? 202 : 200,
      json: { schema_version: '1.0', responseType: 'task', businessStatus: status === 'completed' ? 'partial' : null, taskId, status,
        stage: status === 'running' ? 'refining' : status, requests: 200, networkRequests: 0,
